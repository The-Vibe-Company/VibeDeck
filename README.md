...
- test d’une URL personnalisée avant ajout, avec format détecté, nombre d’éléments et aperçu borné de trois titres, sans rattacher la source au fil ;
...
Les versions sont préparées par Release Please à partir des commits Conventional Commits. Fusionner sa Release PR met à jour la version et le changelog, crée un tag `vX.Y.Z` et une GitHub Release non publique, puis déclenche les builds signés activés. Le workflow signé attache les artefacts validés sans remplacement et publie la release comme `latest` uniquement après contrôle exact du DMG, des métadonnées et des checksums. Une version incomplète n’est donc jamais proposée à l’application. La diffusion est temporairement limitée à macOS pendant la validation Azure Artifact Signing décrite dans [WINDOWS_RELEASE_TODO.md](./WINDOWS_RELEASE_TODO.md). À partir de la version 0.3.0, l’application télécharge les versions stables en arrière-plan et propose explicitement de redémarrer lorsqu’elles sont prêtes.

## Architecture

Le rendu React ne contacte jamais directement les journaux. Le processus principal Electron télécharge et normalise les flux, puis conserve dashboard, panneaux, sources, articles et métadonnées HTTP dans une base SQLite locale. Une même source est mutualisée entre plusieurs panneaux et un échec réseau ne supprime jamais les articles déjà reçus. La recherche utilise un index dérivé et supprimable dans `semantic-search/` ; le modèle E5 et cet index ne modifient jamais `vibedeck.sqlite3` et ne font pas partie des exports.
