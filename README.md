...
- panels « Fil » pour fusionner plusieurs sources dans une chronologie unique ;
- pack de démarrage « Veille concurrents » avec Le Monde, Le Figaro et Le Parisien ;
- catalogue visuel local de 46 sources — 30 médias et 16 sources primaires — présenté dans deux onglets distincts, puis séparé par zone géographique et catégories éditoriales repliables ; chaque source possède une icône locale et une recherche bornée à l’onglet choisi ;
- intervalle initial de 1 minute pour les médias optimisés et de 5 minutes pour les sources primaires, modifiable ensuite dans les réglages du fil ;
...
- reconnaissance directe des 46 sources du catalogue ;
...
Les connecteurs génériques couvrent RSS, Atom et News Sitemap. Le registre validé `electron/publication-registry.mjs` est l’interface unique pour ajouter une source vérifiée ; il dérive automatiquement le catalogue public, la reconnaissance d’URL et les racines proxy exactes. Les médias qui proposent la lecture simplifiée y déclarent aussi leur adaptateur lecteur. La recette détaillée se trouve dans `AGENTS.md` et `CLAUDE.md`.

## Périmètre et droits

Cette V0 sert à valider le produit localement. Les éditeurs et organismes peuvent restreindre l’usage professionnel ou collectif de leurs flux, marques et icônes. Avant un pilote AFP, il faut vérifier les licences de syndication et les conditions d’usage des identités visuelles, ou obtenir les autorisations nécessaires auprès des sources concernées. Aucun contournement de protection anti-bot n’est implémenté.
