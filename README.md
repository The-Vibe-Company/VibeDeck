...
`test:pilot-ui` construit puis pilote Electron sur deux flux RSS et une base temporaires afin de prouver l’interclassement de la baseline, le retour clavier depuis le lecteur, la navigation et le déplacement avec conservation du focus, la géométrie minimale des panels, l’indépendance des tampons partagés et l’absence de déplacement du viewport lors des arrivées. `test:live` interroge réellement les trois sources de lancement et vérifie au plus cinq décisions du lecteur par publication sans journaliser les articles testés. `dist:dir` produit une application locale dans `release/`, `verify:packaged-fuses` lit directement le binaire Electron produit et `test:packaged` lance ce paquet via son ASAR et son protocole interne. La configuration Windows NSIS est prête, mais doit être validée sur une machine ou une CI Windows avant diffusion.
